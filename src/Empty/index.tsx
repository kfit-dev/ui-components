...
const emptyDescription = (description: React.ReactNode, emptyType?: string) => {
  if (emptyType === 'nothing-here')
    return (
      <PresetParagraphs
        paragraphOne={`Nothing here yet, nada.`}
        paragraphTwo={`Check back later, it might appear.`}
      />
    );
  else if (emptyType === 'no-outlet')
    return (
      <PresetParagraphs
        paragraphOne={`Why no outlet eh?`}
        paragraphTwo={`Click on the plus button to add outlets.`}
      />
    );
...
const Empty: React.FC<EmptyProps> = ({
  ...
  return (
    <AntEmpty image={imageProp} description={descriptionProp} {...props} />
  );
};
